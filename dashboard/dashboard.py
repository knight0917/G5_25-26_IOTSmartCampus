...
while True:
    ...
    # Always reload catalog to pick up new rooms immediately
    catalog_config = load_catalog_metadata()
    ...
    with placeholder.container():
        ...
        if not all_rooms:
            ...
            # Responsive Grid: Max 4 columns, but stretch if fewer
            # If 3 rooms -> 3 columns (full width)
            # If 5 rooms -> 4 columns (wrap 5th to next row)
            num_cols = len(all_rooms) if len(all_rooms) < 4 else 4
